...
class Database:
    ...
    def create_session(self, data):
        try: 
            ...
            return  'Exist in database'
        except SQLAlchemyError as e:
            session.rollback()
            logging.error(f"Unexpected error when creating user: {e}")
            return "Unexpected error when creating user: {e}"
        else:
            return 'Add new to database'
        finally:
            session.close()
